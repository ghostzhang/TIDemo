...
function doSetBox(e) {
    ...
    for (var i = 0; i < config.length; i++) {
        ...
        if (t != null) {
            ...
            case 'textarea':
                html += '<div class="box">';
                html += '<label for="' + t[2] + num + '">' + ((t[3] != undefined && t[3] != "") ? t[3] : t[1]) + '：</label>';
                html += '<textarea id="' + t[2] + num + '" placeholder="' + ((t[3] != undefined && t[3] != "") ? "请输入" + t[3] : "") + '">' + (t[4] != undefined ? t[4] : '') + '</textarea>';
                html += '</div>';
                break;
            case 'a':
            case 'link':
...
function doCopy() {
    var note = replaceDemo(demoFile.resolve());alert(note)
    if (note !== "") {
...
var appMain = function() {
        ...
        function loadApp() {
            ...
            $("#pop_set_ok").click(function() {
                ...
                if ($("#set_out_charset_utf8").is(":checked")) {
                    demoFile.outencode = "utf-8";
                }
                if ($("#set_out_charset_gb2312").is(":checked")) {
                    demoFile.outencode = "gb2312";
                }
                $("#floatBoxBg").hide();
                $("#pop_set").hide();
                $("html").css("overflow", "auto");
            });
            $("#pop_set_cancel").click(function() {
                $("#floatBoxBg").hide();
                $("#pop_set").hide();
                $("html").css("overflow", "auto");
            });
