...
function doSave() {
    ...
    var path = demoFile.path;
    var temp_file = name.split(".");
    var suffix = temp_file.pop();
    var file_name = temp_file.length > 1 ? temp_file.join(".") : temp_file;
    var filename = file_name.toString().split("_");
    var path = path.toString().slice(0, path.length - name.length);
    var dir = demoFile.savedir != "" ? demoFile.savedir.toString() : path;
    if (filename.length > 1 && _.first(filename) == "tidemo") {
        filename = _.rest(filename);
...
var appMain = function() {
    ...
    function loadApp() {
        ...
        $("#app-title").text(gAppName + " v" + gAppVersion + " | 2012-2015 by GhostZhang@CSSForest.org");
