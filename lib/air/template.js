    ...
    function file() {
        ...
        this.ext = function() {
            ...
            for (var i = file_config.length - 1; i >= 0; i--) {
                switch (file_config[i].toLocaleUpperCase()) {
                    case "UTF8":
                    case "UTF-8":
                        ch = "utf8"; // 读取编码
                        break;
...
function getQRurl(text) {
    ...
    return qrapi + "?" + bg + fg + gc + el + w + m + pt + inpt + logo + "&text=" + text.replace(/\&/g, '%26').replace(/\r/g, '%0A');
}
...
function usual_search(data, key) {
    // 查找关键字，返回下标
    // air.trace(Date.now()+' '+JSON.stringify(data))
