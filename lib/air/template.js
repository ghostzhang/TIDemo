...
var template = function() {
    function file() {
        ...
        this.ext = function() {
            ...
            var file_config = temp[1].split("::");
            var ch;
            for (var i = file_config.length - 1; i >= 0; i--) {
                switch (file_config[i]) {
                    case "utf8":
                    case "utf-8":
                        ch = file_config[i].toLowerCase(); //读取编码
                        break;
                    case "autosave":
            ...
            if (lp.length > 0) {
                for (var j = 0; j < lp.length; j++) {
                    ...
                    if (list.length > 1) {
                        ...
                        var loop_db = "";
                        var loop_num = !isNaN(lp[j][2]) ? re.test(lp[j][2]) ? lp[j][2].replace(/\\d+/ig, "") : lp[j][2] : 'auto'; // 循环次数
